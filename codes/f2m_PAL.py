...
clahe = cv2.createCLAHE(clipLimit=5.0, tileGridSize=(8, 8)) #Clahe
orb = cv2.ORB_create(nfeatures = 100000) #ORB initialization
...
def get_imgs(fname):
    ...
    Output:
        ...
        depth1: depth map
    """
    fileName1 = '/home/rahul/dreamvu/visual_odo/dataset/carpet_1/depth/fusion_'+str(fname)+'.bin'
    ...
    depth = depth1.ravel()
    my_ind = np.where((depth>=350))[0]
    depth[my_ind] = depth[my_ind]*0
    depth1 = depth.reshape( int(imHeight), int(imWidth))
    # my_ind = np.where(depth1>350)[0]

    img1 = cv2.imread("/home/rahul/dreamvu/visual_odo/dataset/carpet_1/{}.png".format(fname))
    mask1 = ((depth1>0)).astype(int)*250
    
    # Resizing both mask and image to same size
    ...
    #Setting binary range
    mask1 = cv2.inRange(mask1, 150, 255, cv2.THRESH_BINARY)
    # Dialation
    mask1 = cv2.dilate(mask1, np.ones((15,15))) 

    #Outputs the Image, Mask(All points from depth file whose distance is under 3.5 meters), depth_array
...
def get_3d(points2D, depth1, f, cx, cy):
    ...
    # Checking If all points are in less than 3.5 meters range 
    wrong = np.where(Zs[:]>=350)[0]
    Zs[wrong] = 0
    # wrong = np.where(Zs[:]>=350)[0]
    ##

    thetas = (us / (2*cx)).astype(int) * np.pi / 3 # Generating corrosponding thetas
    us_ = us.copy()
    us = us % (2*cx)
    Xs = (us-cx)*Zs/f
    vs_ = vs.copy()
...
def match_features(kp1, kp2, des1, des2):
    ...
    good = []
    for (m,n) in matches:
        if m.distance < 0.9*n.distance: ## Lowe's ratio imp for tuning
            good.append(m)
...
def get_tracking(good, kp_pts3d1, kp_pts3d2, theta):
    ...
    H1,score = cv2.estimateAffinePartial2D(src, dst, ransacReprojThreshold=0.070)
    ...
    while(j<(len(l_fin[0]))):
        ...
        if(i2!=prev):
            ...
            if(check==1):
                ...
                up_bound.append(np.inf)
                up_bound.append(20)
                src_2d[i1].append([int((dst_2d[i2,0])), int((dst_2d[i2,1])),dst_2d[i2,2]])
        j+=1
...
sample_freq = 1 # rate at which Sampling needs to be done
refresh_rate = 3   # rate at which Map refresh needs to be done
