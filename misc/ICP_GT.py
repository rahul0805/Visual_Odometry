...
odometry = np.loadtxt(str("/home/rahul/dreamvu/visual_odo/dataset/carpet_1/turtle_odom.txt"), dtype=np.float32).reshape(-1,3)
my_x = -odometry[:,1]
my_y = odometry[:,0]
my_theta = -odometry[:,2]
...
def read_pcd(i2, target):
    ...
    print(i)
    H = np.eye(4)
    H[0,0] = np.cos(my_theta[3+(i)*3])
    H[0,2] = np.sin(my_theta[3+(i)*3])
    H[2,0] = -np.sin(my_theta[3+(i)*3])
    H[2,2] = np.cos(my_theta[3+(i)*3])
    H[0,3] = my_x[3+(i)*3]
    H[2,3] = my_y[3+(i)*3]
    source.transform(H)
...
o3d.io.write_point_cloud("final_GT.ply", target)
