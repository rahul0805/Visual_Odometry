...
my_x = np.load("../../x_co_f2f.npy")
my_y = np.load("../../y_co_f2f.npy")
my_theta = -np.load("../../theta_co_f2f.npy")
...
# target = o3d.io.read_point_cloud("../../stich/pcd/map_0_100_10.ply")
# i = int((600-200)/100)
# H = np.eye(4)
# H[0,0] = np.cos(my_theta[9+(i)*10])
# H[0,2] = np.sin(my_theta[9+(i)*10])
# H[2,0] = -np.sin(my_theta[9+(i)*10])
# H[2,2] = np.cos(my_theta[9+(i)*10])
# H[0,3] = my_x[9+(i)*10]
# H[2,3] = my_y[9+(i)*10]
# target.transform(H)
updates = []
...
o3d.io.write_point_cloud("final_f2f.ply", target)
